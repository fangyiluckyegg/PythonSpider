import urllib.parse
import urllib.request                             # 用它模拟浏览器的一个请求发起过程
import time
import datetime
...
class Downloader:
    ...
    def download(self, url, headers, proxy, num_retries, data=None):
        ...
        if proxy:
            #通过urllib模块的ProxyHandler函数来支持代理访问
        ...
        try:
            ...
            if hasattr(e, 'code'):
                ...
                if num_retries > 0 and 500 <= code < 600:
                    # retry 5XX HTTP errors（如果遇到5XX类等发生在服务端的问题时，重试下载）
...
class Throttle:
    ...
    def __init__(self, delay):
        # amount of delay between downloads for each domain（获取已设置延迟时间）
        self.delay = delay
        # timestamp of when a domain was last accessed（初始化时间戳）
        self.domains = {}
    ...
    def wait(self, url):
        ...
        last_accessed = self.domains.get(domain) #获取当前当前域名访问时的时间
